...
class TestPermissions(unittest.TestCase):
    def test_base_permissions(self):
        permissions = preconditions.Permissions(
            './options.json')
        self.assertEqual(permissions.get(), defaultPermissions)


class TestOrgPermissions(unittest.TestCase):

    def test_base_permissions(self):
        org_perms = preconditions.OrgPermissions(
            defaultPermissions, "1234567890")
        self.assertEqual(org_perms.permissions, [])

    def test_shared_vpc_permissions(self):
        org_perms = preconditions.OrgPermissions(
            defaultPermissions, "1234567890", shared_vpc=True)
        self.assertEqual(
    ...
    def test_parent_permissions(self):
        org_perms = preconditions.OrgPermissions(
            defaultPermissions, "1234567890", parent=True)
        self.assertEqual(
            org_perms.permissions,
            [
                "resourcemanager.projects.create"
