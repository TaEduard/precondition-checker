...
class OrgPermissions:
    ...
    def __init__(self, permissions, org_id, shared_vpc=False, parent=False):
        """
        ...
        self.permissions = permissions["ALL_PERMISSIONS"][:]

        if self.shared_vpc:
            self.permissions += permissions["SHARED_VPC_PERMISSIONS"]

        if self.parent:
            self.permissions += permissions["PARENT_PERMISSIONS"]
...
class FolderPermissions:
    ...
    def __init__(self, permissions, folder_id, parent=False):
...
class SharedVpcProjectPermissions:
    def __init__(self, permissions, project_id):
        ...
        self.permissions = permissions["SHARED_VPC_PROJ_PERMISSIONS"][:]
...
class SeedProjectServices:
    ...
    def __init__(self, permissions, project_id):
        self.project_id = project_id
        self.permissions = permissions["REQUIRED_APIS"]
    ...
    def validate(self, credentials):
        ...
        enabled = []
        for required_api in self.permissions:
...
class BillingAccount:
    ...
    def __init__(self, permissions, billing_account):
        self.billing_account = billing_account
        self.permissions = permissions["REQUIRED_PERMISSIONS"]
...
def argparser():
    ...
    parser.add_argument(
        '--options_file', required=False, default="./options.json",
        help="""A option file used to list all 
        the permissions/apis required by this 
...
def validators_for(opts, seed_project):
    ...
    """
    validators = []
    permissions = Permissions(opts.options_file).get()

    if seed_project is not None:
        seed_project_validator = SeedProjectServices(permissions, seed_project)
        validators.append(seed_project_validator)

    validators.append(BillingAccount(permissions, opts.billing_account))

    if opts.shared_vpc is not None:
        host_vpc_validator = SharedVpcProjectPermissions(
            permissions, opts.shared_vpc)
    ...
    if opts.folder_id is not None:
        validators.append(FolderPermissions(
            permissions, opts.folder_id, parent=True))
        org_parent = False
    else:
        org_parent = True
